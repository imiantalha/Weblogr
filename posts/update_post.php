...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['blog_id'], $_POST['title'], $_POST['description'])) {
        ...
        if (isset($_FILES['uploadimage']) && $_FILES['uploadimage']['error'] === UPLOAD_ERR_OK) {
            ...
            $sql = "UPDATE `blogs` SET `title`='$title', `created_date`= NOW(), `image_path`='$filename', `description`='$description' WHERE `blog_id`='$blog_id'";
        } else {
            // Update the blog post without changing the image path
            $sql = "UPDATE `blogs` SET `title`='$title', `created_date`= NOW(), `description`='$description' WHERE `blog_id`='$blog_id'";
        }
